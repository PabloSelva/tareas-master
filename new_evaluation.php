		...
		<div class="card-body">
			<form action="" id="manage_evaluation">
				...
				<div class="row">
					<div class="col-md-6 border-right">
						<div class="form-group">
							...
							<select name="employee_id" id="employee_id" class="form-control form-control-sm select2">
								<option value=""></option>
								<?php 
								$employees = $conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM lista_miembros where evaluator_id = {$_SESSION['login_id']} order by concat(lastname,', ',firstname,' ',middlename) asc");
						...
						<div class="form-group">
							...
							<select name="task_id" id="task_id" class="form-control form-control-sm" required>
								<option value="" disabled="" selected="">Por favor seleccione Empleado primero.</option>
							</select>
						</div>
						<div class="row " id="ratings-field" style="display: none">
							<div class="col-md-12">
							<label for="" class="control-label">Calificaciones</label>
							</div>
							<hr>
							<div class="col-md-6">
								<div class="form-group">
									<label for="" class="control-label">Eficiencia</label>
								...
								<div class="form-group">
									<label for="" class="control-label">Tiempo</label>
							...
							<div class="col-md-6">
								<div class="form-group">
									<label for="" class="control-label">Calidad</label>
								...
								<div class="form-group">
									<label for="" class="control-label">Precisión</label>
					...
					<div class="col-md-6">
						<div id="post-field">
							<div><center><i>Selecciona una tarea primero</i></center></div>
						</div>
					</div>
				</div>
				<hr>
				<div class="col-lg-12 text-right justify-content-center d-flex">
					...
					<button class="btn btn-secondary" type="button" onclick="location.href = 'index.php?page=evaluation'">Cancelar</button>
				</div>
			</form>
		</div>
	</div>
...
<script>
	...
	function update_emp(){
		...
		$.ajax({
					 ...
					 if(Object.keys(resp).length > 0){
					 	...
					 	Object.keys(resp).map(k=>{
							...
							if(task_id == resp[k].id)
					 		...
					 		$('#task_id').html('')
					 	var oc = opt.clone()
					 		oc.text("El empleado aún no está asignado a ninguna tarea.")
			...
			complete:function(){
				$('#task_id').select2({
					placeholder:'Por favor selecciona tarea acá',
					width:'100%'
				})
				task_update()
				end_load();
	...
	$('#manage_evaluation').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					alert_toast('Datos Grabados',"proceso exitoso");
					setTimeout(function(){

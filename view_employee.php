...
<?php
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM lista_miembros where id = ".$_GET['id'])->fetch_array();
foreach($qry as $k => $v){
	$$k = $v;
}
$designation= $conn->query("SELECT * FROM lista_cargos where id = $designation_id ");
$designation = $designation->num_rows > 0 ? $designation->fetch_array()['designation'] : 'Cargo Desconocido';
$department= $conn->query("SELECT * FROM lista_departamentos where id = $department_id ");
$department = $department->num_rows > 0 ? $department->fetch_array()['department'] : 'Departamento Desconocido';
$evaluator= $conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM lista_evaluadores where id = $evaluator_id ");

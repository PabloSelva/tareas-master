...
<script type="text/javascript">
    ...
    google.charts.setOnLoadCallback(drawGeneralPieChart);
    google.charts.setOnLoadCallback(drawUserPieChart);
    google.charts.setOnLoadCallback(drawBarChart);

    function drawGeneralPieChart() {
    ...
    function drawUserPieChart() {
        var data = google.visualization.arrayToDataTable([
            ...
            <?php
            $conexion = mysqli_connect("localhost", "root", "", "empleado");
            $user_id = $_SESSION['login_id']; // Obtener el ID del usuario logeado de la sesión
            $SQL = "SELECT status AS estado, COUNT(*) as cantidad FROM lista_tareas WHERE employee_id = $user_id GROUP BY estado";
            $consulta = mysqli_query($conexion, $SQL);
...
<?php if ($_SESSION['login_type'] == 2) : ?>
    ...
    <h1>Gráfico: Estado general de las tareas</h1>
    <div id="piechart_3d" style="margin: 20px  ; width: 500px; height: 200px;"></div>

    <h1>Gráfico: Tareas asignadas por miembro</h1>
    <div id="barchart_material" style="margin: 10px auto; width: 1000px; height: 500px; "></div>

<?php else : ?>
    <div class="col-12">
        <div class="card">
            <div class="card-body">
              <h2>Bienvenid@ <?php echo $_SESSION['login_name'] ?>!</h2>
            </div>
        </div>
    </div>

...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT r.*,concat(e.lastname,', ',e.firstname,' ',e.middlename) as name,t.task,concat(ev.lastname,', ',ev.firstname,' ',ev.middlename) as ename,((((r.efficiency + r.timeliness + r.quality + r.accuracy)/4)/5) * 100) as pa FROM ratings r inner join lista_miembros e on e.id = r.employee_id inner join lista_tareas t on t.id = r.task_id inner join lista_evaluadores ev on ev.id = r.evaluator_id  where r.id = ".$_GET['id'])->fetch_array();
	foreach($qry as $k => $v){
		$$k = $v;
	}
}
	...
	<div class="col-lg-12">
		<div class="row">
			<div class="col-md-6">
				...
				<dl>
					<dt><b class="border-bottom border-primary">Evaluador</b></dt>
					<dd><?php echo ucwords($ename) ?></dd>
				</dl>
				<dl>
					<dt><b class="border-bottom border-primary">Fecha de Evaluación</b></dt>
			...
			<div class="col-md-6">
				...
				<dl>
					<dt><b class="border-bottom border-primary">Cualidad</b></dt>
					<dd><?php echo $quality ?></dd>
				</dl>
				<dl>
					<dt><b class="border-bottom border-primary">Precisión</b></dt>
